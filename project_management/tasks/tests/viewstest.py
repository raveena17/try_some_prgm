...
class TasksTest(TestCase):
    ...
    def test_create_task(self):        
        ...
        response = self.client.post('/task/save/?pid=' + str(self.test_project.id),
            {
            'name': 'sample task',
    ...
    def test_delete_task(self):       
        ...
        response = self.client.post('/task/delete/', {'task_pk': ['1']})
    ...
    def test_list_task(self):        
        ...
        response = self.client.post('/task/list/?pid=' + str(project_list[0].id))
        self.assertEquals(response.status_code, 200)
        task_list = Task.objects.all()
        self.assertEquals(len(task_list), 1)
        #search text
        response = self.client.post('/task/list/?pid=' 
    ...
    def test_exists_task_check(self):
        ...
        self.assertEquals(errors[0].message.title(),u'Task Name Already Exists.') 
...
class TaskTypeTest(TestCase):
    ...
    def test_create_tasktype(self):        
        response = self.client.post('/task/create_type/',
                                    {
                                    'name': 'Testing'
                                    })
        self.assertEquals(response.status_code, 200)
    ...
    def test_delete_tasktype(self):       
        self.test_create_tasktype()
        response = self.client.post('/task/delete_type/1/')
        self.assertEquals(response.status_code, 200)
        type_list = Type.objects.all()
...
class NonProjectTaskTest(TestCase):
    ...
    def test_create_nonproject_task(self):        
        ...
        task_type = Type.objects.get(pk=1)
        response = self.client.post('/task/nonproject/create/',
                                    {
                                    'name': 'Learning Flask',
                                    'start_date': '03-06-2013',
                                    'end_date': '03-09-2013',
    ...
    def test_update_nonproject_task(self):       
        ...
        task_type = Type.objects.get(pk=1)
        task_list = Task.objects.all()
        response = self.client.post('/task/nonproject/update/' + str(task_list[0].id) + '/',
                                    {
                                    'name': 'Flask learning',
                                    'start_date': '03-06-2013',
                                    'end_date': '03-09-2013',
    ...
    def test_list_nonproject_task(self):        
        self.test_create_nonproject_task()
        response = self.client.post('/task/nonproject/list/')
        self.assertEquals(response.status_code, 200)
        task_list = Task.objects.all()
        self.assertEquals(len(task_list), 1)
        response = self.client.post('/task/nonproject/list/?search=Flask')
        self.assertEquals(response.status_code, 200)
